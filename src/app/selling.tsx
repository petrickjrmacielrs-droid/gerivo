...
type SellingKitDraft = { kitId: string; name: string; visualName: string; description: string; isTire: boolean; maxInstallments: number; active: boolean; displayOrder: number; scopeType: "GLOBAL" | "GROUP" | "COMPANY"; targetGroupId: string; targetCompanyId: string; items: PackageDraftItem[] };
...
export function MasterSellingManager({ sessionAccessToken }: { sessionAccessToken: string }) {
  ...
  async function saveCatalogKit() {
    ...
    setSaving(true); setError(""); setNotice("");
    try {
      const response = await fetch("/api/master/selling", { method:"POST", headers:{"Content-Type":"application/json",Authorization:`Bearer ${sessionAccessToken}`}, body:JSON.stringify({ action:"save-kit", kitId:kitDraft.kitId || undefined, name:kitDraft.name, visualName:kitDraft.visualName, description:kitDraft.description, isTire:kitDraft.isTire, maxInstallments:kitDraft.maxInstallments, active:kitDraft.active, displayOrder:kitDraft.displayOrder, targetGroupId:kitDraft.scopeType === "GROUP" ? kitDraft.targetGroupId : null, targetCompanyId:kitDraft.scopeType === "COMPANY" ? kitDraft.targetCompanyId : null, items:kitDraft.items }) });
...
export function SellingOperationPage({ companyId, storeId, accessToken, customers, vehicles, currentUserName, currentUserPhone, storeName, companyName, identity }: SellingOperationProps) {
  ...
  function maxInstallmentsFor(total: number, pkg?: SellingPackage) {
    ...
    const tireBoost = pkg ? (extraItemsByPackage[pkg.id] || []).filter((item)=>item.is_tire).reduce((max,item)=>Math.max(max, Number(item.max_installments)||1), 1) : 1;
